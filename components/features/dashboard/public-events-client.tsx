...
export function PublicEventsClient({
  ...
  const openModal = (e: React.MouseEvent, event: EventItem) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  return (
    <>
      <div className="space-y-6">
        ...
        <p className="text-xs text-pri-silver/50 text-center">
              ...
              return (
                ...
                >
                  <Link href={`/events/${event.slug}`} className="block h-full">
                    <Card className="glass-tech overflow-hidden group h-full flex flex-col cursor-pointer hover:border-pri-red/30 transition-all duration-300">
                      ...
                      <div className="relative h-48 overflow-hidden">
                        ...
                        <div className="absolute bottom-3 left-3 z-10">
                          <div className="flex items-center gap-1.5 text-xs text-white bg-black/50 backdrop-blur-sm px-2.5 py-1 rounded-full">
                        ...
                        {/* Hover Overlay */}
                        <div className="absolute inset-0 bg-pri-red/0 group-hover:bg-pri-red/10 transition-all duration-300" />
                      </div>
                      ...
                      <CardContent className="p-5 flex-1 flex flex-col">
                        ...
                        <div className="space-y-2 mb-4 flex-1">
                          <div className="flex items-center gap-2 text-xs text-pri-silver">
                            ...
                            <span>
                              ...
                              {new Date(event.end_date).toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit" })} WIB
                            </span>
                          </div>
                          {event.location && (
                            <div className="flex items-center gap-2 text-xs text-pri-silver">
                        ...
                        {/* Action Buttons */}
                        <div className="flex gap-2 mt-auto pt-3 border-t border-white/5" onClick={(e) => e.stopPropagation()}>
                          {isOpen && (
                            <Button
                            ...
                            >
                              {isRegistered ? (
                                <><CheckCircle className="h-3.5 w-3.5 mr-1" />Terdaftar</>
                              ) : (
                                <><CheckCircle className="h-3.5 w-3.5 mr-1" />Daftar</>
                              )}
                            </Button>
                          )}
                          <Link href={`/events/${event.slug}`} className="flex-1" onClick={(e) => e.stopPropagation()}>
                            <Button
                              variant="outline"
                              size="sm"
                              className="border-white/10 text-pri-silver hover:text-white w-full h-9 text-xs group/btn"
                            >
                              <ArrowRight className="h-3.5 w-3.5 mr-1 transition-transform group-hover/btn:translate-x-0.5" />
                              Detail
                            </Button>
                          </Link>
                        </div>
