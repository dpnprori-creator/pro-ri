...
import { registerForProgram, cancelProgramRegistration } from "@/features/admin/programs-actions";
...
export const ProgramRegistration = ProgramRegistrationButton;
...
export function ProgramRegistrationButton({
  ...
  const handleCancel = async () => {
    ...
    setLoading(true);
    const result = await cancelProgramRegistration(programId);
    setLoading(false);

    if (result.error) {
      toast.error(result.error);
    } else {
      toast.success(result.message || "Pendaftaran dibatalkan");
      router.refresh();
    }
  };
    ...
    return (
      <div className="space-y-3">
        <div className="flex items-center gap-2 p-3 rounded-lg bg-green-500/10 border border-green-500/20">
          ...
          <Badge variant="success" className="ml-auto">Terdaftar</Badge>
        </div>
        <Button
